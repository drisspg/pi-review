...
2. PR open/refresh/cleanup is serialized per PR; checkout preparation is serialized per shared clone. A different checkout HEAD or an existing checkout on cleanup now refuses before invalidating analyses or disposing sessions. Checkout deletion requires explicit offline cache maintenance, excluded by the server's lifetime cache lock. After successful preparation, a new registered revision invalidates old analysis observers and SDK queues and disposes SDK sessions/terminals before publishing context. Sending a termination signal alone is not proof that a terminal exited.
...
5. A focus scan is clean only after a successful explicit `No focus areas found.` conclusion with no parsed findings. Preceding investigation notes are preserved in the UI. Unparseable output is invalid, never clean. Generated focus/guide locations are HEAD-file navigation references: exact current changed-file paths (not deleted files) and positive safe integer line ranges. Surrounding unchanged code and ranges spanning hunks are allowed; missing patches do not invalidate navigation. The captured diff cannot establish full-file EOF bounds, so prompts require inspecting HEAD text before citing unseen lines, while the UI loads that text on demand. Do not reuse GitHub comment-anchor validation for analysis: actual inline comments still require reviewable endpoints within a single diff hunk. Raw invalid output remains in run diagnostics.
