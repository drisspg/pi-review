...
type DiffProps = {
  ...
  askFocusArea: (area: FocusArea, question: string) => Promise<string>;
  sideWidth: number;
...
function App() {
  ...
  async function askFocusArea(area: FocusArea, question: string): Promise<string> {
...
function DraftView({ draft, drafts, setDrafts, editingDraftId, setEditingDraftId }: { draft: DraftComment; drafts: DraftComment[]; setDrafts: (drafts: DraftComment[]) => void; editingDraftId: string | null; setEditingDraftId: (id: string | null) => void }) {
  ...
  return <div className={`comment draft-card pending-draft${editing ? " editing" : ""}${removing ? " removing" : ""}`}><div className="pending-draft-meta"><strong>Pending draft</strong><span>{targetLabel(draft)}</span></div><div className="pending-draft-body">{editing ? <textarea autoFocus value={draft.body} onChange={(event) => updateDraft(drafts, setDrafts, draft.id, event.target.value)} /> : <p>{draft.body}</p>}</div><div className="actions pending-draft-actions"><button className="draft-icon-button edit-draft-button" title={editing ? "Done editing" : "Edit draft"} onClick={() => setEditingDraftId(editing ? null : draft.id)}>{editing ? "Done" : "Edit"}</button><button className="draft-icon-button remove-draft-button" onClick={removeDraft} disabled={removing}>{removing ? "Removing…" : "Remove"}</button></div></div>;
}
...
function FocusAreaInline({ area, active, setActiveFocusAreaId, collapsedFocusAreaIds, setCollapsedFocusAreaIds, askFocusArea, addDraft }: { area: FocusArea; active: boolean; setActiveFocusAreaId: (id: string | null) => void; collapsedFocusAreaIds: Record<string, boolean>; setCollapsedFocusAreaIds: DiffProps["setCollapsedFocusAreaIds"]; askFocusArea: (area: FocusArea, question: string) => Promise<string>; addDraft: (body: string) => void }) {
...
function AiReviewPanel({ review, setReview, runReview, sendMessage, focusReview, runFocusReview, focusAreas, collapsedFocusAreaIds, setCollapsedFocusAreaIds }: { review: AiReview; setReview: (review: AiReview) => void; runReview: () => Promise<void>; sendMessage: (message: string) => Promise<void>; focusReview: FocusReview; runFocusReview: () => Promise<void>; focusAreas: FocusArea[]; collapsedFocusAreaIds: Record<string, boolean>; setCollapsedFocusAreaIds: DiffProps["setCollapsedFocusAreaIds"] }) {
  const [draft, setDraft] = useState("");
  const focusAreaCount = focusAreas.length;
  const allFocusAreasCollapsed = focusAreaCount > 0 && focusAreas.every((area) => collapsedFocusAreaIds[area.id]);
  ...
  function toggleFocusAreas(): void {
    setCollapsedFocusAreaIds(Object.fromEntries(focusAreas.map((area) => [area.id, !allFocusAreasCollapsed])));
  }
  ...
  return <>
    <section className="panel ai-review">
      <div className="thread-head"><h2>Pi review</h2><div className="actions"><Button className="focus-review-run" variant="muted" onClick={() => void runFocusReview()} disabled={focusReview.running}>{focusReview.running ? "Scanning…" : focusAreaCount > 0 ? `Focus scan (${focusAreaCount})` : "Focus scan"}</Button><Button variant="muted" onClick={() => setReview({ ...review, expanded: true, open: true })} disabled={!hasMessages && !review.open}>Focus</Button><Button variant="muted" onClick={() => setReview({ ...review, open: !review.open })}>{review.open ? "Collapse" : "Show"}</Button>{focusAreaCount > 0 && <Button variant="muted" onClick={toggleFocusAreas}>{allFocusAreasCollapsed ? "Expand focus" : "Collapse focus"}</Button>}</div></div>
      <Button onClick={() => void runReview()} disabled={review.running}>{review.running ? "Reviewing…" : hasMessages ? "Run again" : "Run review"}</Button>
      {focusReview.text.length > 0 && (focusAreaCount > 0 ? <p className="focus-review-note">{focusAreaCount} focus area{focusAreaCount === 1 ? "" : "s"}{allFocusAreasCollapsed ? " collapsed inline." : " highlighted inline."}</p> : <p className="focus-review-note clean">Focus scan clean: nothing specific to dig into.</p>)}
