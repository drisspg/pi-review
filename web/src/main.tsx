...
const GitHubDraftContext = createContext<GitHubDraftControls>({
  review: null,
  loaded: false,
  loading: false,
  moving: false,
...
function githubDraftLocation(comment: GitHubDraftComment): string {
  return `${comment.path}:${comment.line == null ? "file" : comment.startLine != null && comment.startLine !== comment.line ? `${comment.startLine}-${comment.line}` : comment.line}`;
}

function githubDraftTarget(files: PullFile[], comment: GitHubDraftComment): Target {
  if (comment.line == null) return { path: comment.path, line: null, side: "RIGHT", hunk: "" };
  const rows = parsePatchRows(files.find((file) => file.filename === comment.path)?.patch);
  const row = rows.find((candidate) => candidate.newLine === comment.line) ?? rows.find((candidate) => candidate.oldLine === comment.line);
  const side = row?.newLine === comment.line ? "RIGHT" : "LEFT";
  return { path: comment.path, line: comment.line, startLine: comment.startLine, side, hunk: row?.hunk ?? "" };
}

function githubDraftHandoffPayload(review: OpenResponse, pendingReview: GitHubPendingReview): Record<string, unknown> {
  return {
    ...
    comments: pendingReview.comments.map((comment) => {
      const target = githubDraftTarget(review.files, comment);
      return { path: comment.path, line: comment.line, startLine: comment.startLine, body: comment.body, diffHunk: target.hunk || "Patch hunk unavailable." };
    }),
  };
}
...
function reviewActionPayload(pr: StoredPullRequest, drafts: DraftComment[], event: "COMMENT" | "APPROVE" | "REQUEST_CHANGES", body: string): Record<string, unknown> {
  return {
    prUrl: pr.url,
    headSha: pr.headSha,
    event,
    body,
    comments: drafts.filter((draft) => draft.line != null).map(({ id, path, line, startLine, side, body: commentBody }) => ({ draft_id: id, path, line, side, body: commentBody, ...(startLine != null && startLine !== line ? { start_line: startLine, start_side: side } : {}) })),
  };
}
...
function App() {
  ...
  const [reviewMode, setReviewMode] = useState<"overview" | "guide" | "diff">("diff");
  ...
  useEffect(() => {
    function beginWindowDrag(event: MouseEvent) {
      ...
      if (target != null && event.button === 0 && !isDiffCodeTarget(event.target)) {
        beginDrag(target);
        event.preventDefault();
        return;
      }
      if (event.target instanceof Element && event.target.closest(".inline-thread") == null) collapseThreads();
    }
  ...
  useEffect(() => {
    if (review == null || (review.draftReview == null && reviewEvent === "COMMENT" && reviewBody.length === 0 && drafts.length === 0)) {
      setDraftSaveStatus("idle");
      setDraftSaveError(null);
      return;
    }
    ...
    const timeout = window.setTimeout(() => {
      void api<{ draftReview: OpenResponse["draftReview"] }>("/api/draft-review/save", { method: "POST", body: JSON.stringify({ prKey: review.pr.key, headSha: review.pr.headSha, event: reviewEvent, body: reviewBody, comments: drafts }), signal: controller.signal })
        .then(({ draftReview }) => {
          updateCachedReview(review.pr.key, (current) => ({ ...current, draftReview }));
          if (requestId === draftSaveRequestIdRef.current) setDraftSaveStatus("saved");
        })
        .catch((err: unknown) => {
  ...
  async function pullGithubDraftReview() {
    if (review == null || githubDraftLoading) return;
    setGithubDraftLoading(true);
    setGithubDraftError(null);
    try {
      const response = await api<{ review: GitHubPendingReview | null }>("/api/github-draft-review/pull", { method: "POST", body: JSON.stringify({ prUrl: review.pr.url }) });
  ...
  async function copyGithubDraftHandoff() {
    if (review == null || githubDraftReview == null || githubDraftReview.comments.length === 0) return;
    const { prompt } = await buildPiPrompt(githubDraftHandoffPayload(review, githubDraftReview));
  ...
  function collapseThreads() {
    setThreads((current) => Object.values(current).some((thread) => !thread.collapsed)
      ? Object.fromEntries(Object.entries(current).map(([key, thread]) => [key, { ...thread, collapsed: true }]))
      : current);
    setActiveTarget(null);
  }
  ...
  function openThread(target: Target) {
    ...
    setThreads({ ...Object.fromEntries(Object.entries(threads).map(([threadKey, thread]) => [threadKey, threadKey === key ? thread : { ...thread, collapsed: true }])), [key]: existing == null ? { key, target, collapsed: false, terminalOpen: false } : { ...existing, collapsed: !existing.collapsed } });
  ...
  function clearActiveDraftReview(prKey: string): void {
    draftSaveAbortRef.current?.abort();
    draftSaveRequestIdRef.current += 1;
    updateCachedReview(prKey, (current) => ({ ...current, draftReview: null }));
    setDrafts([]);
    setReviewBody("");
  ...
  async function archiveReview(event: "COMMENT" | "APPROVE" | "REQUEST_CHANGES", body: string): Promise<boolean> {
    if (review == null) return false;
    try {
      await api("/api/review/archive", { method: "POST", body: JSON.stringify(reviewActionPayload(review.pr, drafts, event, body)) });
  ...
  async function discardReview(): Promise<boolean> {
    if (review == null) return false;
    try {
      await api("/api/draft-review/discard", { method: "POST", body: JSON.stringify({ prKey: review.pr.key }) });
      clearActiveDraftReview(review.pr.key);
      return true;
    } catch (err) {
      setError(err instanceof Error ? err.message : String(err));
  ...
  async function submitReview(event: "COMMENT" | "APPROVE" | "REQUEST_CHANGES", body: string): Promise<boolean> {
    ...
    setSubmitting(true);
    try {
      const { pr } = await api<{ pr: StoredPullRequest | null }>("/api/review/submit", { method: "POST", body: JSON.stringify(reviewActionPayload(review.pr, drafts, event, body)) });
      clearActiveDraftReview(review.pr.key);
  ...
  async function runFlowDag() {
    ...
    try {
      ...
      const setAnswer = (answer: string) => {
        if (activeReviewKeyRef.current !== targetReview.pr.key) return;
        setFlowDag((current) => ({ running: true, text: "", error: null, activity: streamingAgentActivity(current.activity, answer) }));
      };
      const answer = await askPiApi({ prKey: targetReview.pr.key, prompt, purpose }, setAnswer);
      cancelActivityPolling();
      if (activeReviewKeyRef.current !== targetReview.pr.key) return;
  ...
  return <ThemeProvider colorMode={primerColorMode} dayScheme="light" nightScheme={primerNightScheme}>
    ...
    <AppToolbar
      ...
      openCodeWalk={() => { setReviewMode("overview"); if (flowDag.text.trim().length === 0 && !flowDag.running) void runFlowDag(); }}
...
function ReviewPage({ threads, setActiveFocusAreaId, ...props }: DiffProps & { reviewMode: "overview" | "guide" | "diff"; setReviewMode: (mode: "overview" | "guide" | "diff") => void; codeWalk: FlowDag; runCodeWalk: () => Promise<void>; draftRevealId: string | null; piPanel: PiPanelProps; reviewEvent: "COMMENT" | "APPROVE" | "REQUEST_CHANGES"; setReviewEvent: (event: "COMMENT" | "APPROVE" | "REQUEST_CHANGES") => void; reviewBody: string; setReviewBody: (body: string) => void; draftSaveStatus: "idle" | "saving" | "saved" | "error"; draftSaveError: string | null; retryDraftSave: () => void; archiveReview: (event: "COMMENT" | "APPROVE" | "REQUEST_CHANGES", body: string) => Promise<boolean>; discardReview: () => Promise<boolean>; submitReview: (event: "COMMENT" | "APPROVE" | "REQUEST_CHANGES", body: string) => Promise<boolean>; submitting: boolean; invalidDraftIds: Record<string, boolean>; refreshingActivity: boolean; githubDrafts: GitHubDraftControls }) {
  ...
  const guideChapters = useMemo(() => parseGuideChapters(props.piPanel.guideReview.text), [props.piPanel.guideReview.text]);
  ...
  function selectReviewMode(mode: "overview" | "guide" | "diff"): void {
    props.setReviewMode(mode);
    if (mode === "overview" || mode === "guide") {
      setSideFocused(false);
      setSideCollapsed(true);
    }
    if (mode === "overview" && props.codeWalk.text.trim().length === 0 && !props.codeWalk.running) void props.runCodeWalk();
    if (mode === "guide" && props.piPanel.guideReview.text.trim().length === 0 && !props.piPanel.guideReview.running) void props.piPanel.runGuideReview();
  }
  ...
  function prepareGuideStep(step: FocusArea): void {
    if (props.openFiles[step.path] === false) props.setOpenFiles({ ...props.openFiles, [step.path]: true });
  }
  function renderGuideDiff(step: FocusArea): ReactNode {
    const file = props.review.files.find((candidate) => candidate.filename === step.path);
    if (file == null) return <Flash variant="danger">The guide references {step.path}, but that file is not in the current diff.</Flash>;
    return <DiffAnnotationsContext.Provider value={annotations}><FileDiff key={file.filename} file={file} {...props} /></DiffAnnotationsContext.Provider>;
  }
    ...
    <div className={`review-layout${sideCollapsed ? " side-collapsed" : ""}${sideFocused ? " side-focused" : ""}`} style={{ gridTemplateColumns }}>
      <div className="review-main">
        ...
        <nav className="review-mode-tabs" aria-label="Review view">
          <button type="button" className={reviewMode === "overview" ? "active" : ""} aria-pressed={reviewMode === "overview"} onClick={() => selectReviewMode("overview")}>Overview</button>
        ...
        {reviewMode === "overview" ? <CodeWalkPage review={props.review} codeWalk={props.codeWalk} runCodeWalk={props.runCodeWalk} /> : reviewMode === "guide" ? <GuideReview review={props.review} chapters={guideChapters} viewedIds={viewedGuideIds} setViewedIds={setViewedGuideIds} guideReview={props.piPanel.guideReview} runGuideReview={props.piPanel.runGuideReview} onActivate={prepareGuideStep} renderDiff={renderGuideDiff} /> : <main className="files">
...
/** Collapse markdown source to plain words so collapsed previews don't show `**` / `#` syntax. */
function markdownPreviewText(markdown: string, maxWords: number): string {
  ...
  return <section className={`pr-summary github-thread${collapsed ? " minimized" : ""}`}>
    <div className="thread-head">
      <div className="thread-title">
        ...
        <div><strong>Description</strong><span>{summary == null || summary.length === 0 ? "No description provided" : markdownPreviewText(summary, 12)}</span></div>
      </div>
    </div>
...
/** Render the show-me-style visual orientation as a first-class PR view. */
function CodeWalkPage({ review, codeWalk, runCodeWalk }: { review: OpenResponse; codeWalk: FlowDag; runCodeWalk: () => Promise<void> }) {
  return <main className="code-walk-page" aria-label="PR overview">
    <header className="code-walk-page-head">
      <div><span className="guide-kicker">Overview</span><h2>{review.pr.title}</h2><p>Visual orientation for the change: its goal, execution path, ownership boundaries, and the smallest useful reviewer route.</p></div>
      <div>{codeWalk.running && <AgentActivityLine activity={codeWalk.activity} />}<Button variant="muted" className="small-muted-button" onClick={() => void runCodeWalk()} disabled={codeWalk.running}>{codeWalk.running ? "Building…" : codeWalk.text.trim().length > 0 ? "Refresh overview" : "Build overview"}</Button></div>
    </header>
    <section className="code-walk-page-body">
      {codeWalk.error != null ? <Flash variant="danger">Overview failed: {codeWalk.error}</Flash> : codeWalk.running ? <div className="code-walk-loading"><span className="spinner" /><p>Building a focused visual explanation of this PR…</p></div> : codeWalk.text.trim().length > 0 ? <InlineSnippetsProvider value={{ headSha: review.pr.headSha, snippets: false }}><MarkdownText text={codeWalk.text} fileLinks={{ prUrl: review.pr.url }} /></InlineSnippetsProvider> : <div className="guide-empty-card"><h2>Show me what’s going on</h2><p>Build a concise visual explanation before reviewing the implementation.</p><Button onClick={() => void runCodeWalk()}>Build overview</Button></div>}
    </section>
  </main>;
}
...
function GuideReview({ review, chapters, viewedIds, setViewedIds, guideReview, runGuideReview, onActivate, renderDiff }: { review: OpenResponse; chapters: GuideChapter[]; viewedIds: Record<string, boolean>; setViewedIds: React.Dispatch<React.SetStateAction<Record<string, boolean>>>; guideReview: FocusReview; runGuideReview: () => Promise<void>; onActivate: (step: FocusArea) => void; renderDiff: (step: FocusArea) => ReactNode }) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      document.querySelectorAll(".guide-step-highlight").forEach((row) => row.classList.remove("guide-step-highlight"));
    };
  }, [active?.step.id]);
...
function PatchSetRows({ file, sections, setThreads, drafts, setDrafts, editingDraftId, setEditingDraftId, dragSelection, beginDrag, updateDrag, finishDrag, handleRowClick, prUrl, refreshGithubActivity, collapseSignal, commentsCollapsed, diffViewMode, activeFocusAreaId, collapsedFocusAreaIds, setCollapsedFocusAreaIds }: { file: PullFile; sections: ReturnType<typeof parsePatchSetSections>; setThreads: DiffProps["setThreads"]; drafts: DraftComment[]; setDrafts: (drafts: DraftComment[]) => void; editingDraftId: string | null; setEditingDraftId: (id: string | null) => void; dragSelection: DragSelection | null; beginDrag: (target: Target) => void; updateDrag: (target: Target) => void; finishDrag: (target: Target) => void; handleRowClick: (target: Target, extend: boolean) => void; prUrl: string; refreshGithubActivity: () => Promise<void>; collapseSignal: number; commentsCollapsed: boolean; diffViewMode: DiffViewMode; activeFocusAreaId: string | null; collapsedFocusAreaIds: Record<string, boolean>; setCollapsedFocusAreaIds: DiffProps["setCollapsedFocusAreaIds"] }) {
...
function FoldedRows({ file, rows, setThreads, expandedNeighborRows, expandNeighbor, drafts, setDrafts, editingDraftId, setEditingDraftId, dragSelection, beginDrag, updateDrag, finishDrag, handleRowClick, prUrl, refreshGithubActivity, collapseSignal, commentsCollapsed, diffViewMode, activeFocusAreaId, collapsedFocusAreaIds, setCollapsedFocusAreaIds }: { file: PullFile; rows: DiffRow[]; setThreads: DiffProps["setThreads"]; expandedContext: Record<string, boolean>; setExpandedContext: (expanded: Record<string, boolean>) => void; expandedNeighborRows: Record<string, DiffRow[]>; expandNeighbor: (file: PullFile, key: string, startLine: number, endLine: number) => Promise<void>; drafts: DraftComment[]; setDrafts: (drafts: DraftComment[]) => void; editingDraftId: string | null; setEditingDraftId: (id: string | null) => void; dragSelection: DragSelection | null; beginDrag: (target: Target) => void; updateDrag: (target: Target) => void; finishDrag: (target: Target) => void; handleRowClick: (target: Target, extend: boolean) => void; prUrl: string; refreshGithubActivity: () => Promise<void>; collapseSignal: number; commentsCollapsed: boolean; diffViewMode: DiffViewMode; activeFocusAreaId: string | null; collapsedFocusAreaIds: Record<string, boolean>; setCollapsedFocusAreaIds: DiffProps["setCollapsedFocusAreaIds"] }) {
...
function isDiffCodeTarget(target: EventTarget | null): boolean {
  return target instanceof Element && target.closest(".code-cell code") != null;
}

function hasSelectedDiffCode(event: React.MouseEvent<HTMLElement>): boolean {
  return isDiffCodeTarget(event.target) && (window.getSelection()?.toString().length ?? 0) > 0;
}
...
function DraftView({ draft, index, invalid = false, drafts, setDrafts, editingDraftId, setEditingDraftId, onJump }: { draft: DraftComment; index?: number; invalid?: boolean; drafts: DraftComment[]; setDrafts: (drafts: DraftComment[]) => void; editingDraftId: string | null; setEditingDraftId: (id: string | null) => void; onJump?: () => void }) {
  ...
  return <div className={`draft-card${editing ? " editing" : ""}${removing ? " removing" : ""}${invalid ? " invalid" : ""}`}>
    <div className="draft-card-head">
      <div className={`draft-card-location${onJump != null ? " clickable" : ""}`} onClick={onJump} onKeyDown={(event) => { if ((event.key === "Enter" || event.key === " ") && onJump != null) { event.preventDefault(); onJump(); } }} role={onJump != null ? "button" : undefined} tabIndex={onJump != null ? 0 : undefined}>
    ...
    <div className="draft-card-body">{editing ? <MarkdownEditor autoFocus value={draft.body} onChange={(body) => updateDraft(drafts, setDrafts, draft.id, body)} ariaLabel="Edit draft comment" /> : <MarkdownText text={draft.body} />}</div>
  </div>;
}
...
function ThreadBox({ thread, setThread, removeThread, addDraft }: { thread: Thread; setThread: (thread: Thread) => void; removeThread: () => void; addDraft: (body: string) => void }) {
  ...
  async function deleteThread(): Promise<void> {
    ...
    if (review == null || deleting) return;
    setDeleting(true);
    setDeleteError(null);
    try {
...
function ReviewSummary({ pr, files, drafts, setDrafts, event, setEvent, body, setBody, draftSaveStatus, draftSaveError, retryDraftSave, editingDraftId, setEditingDraftId, archiveReview, discardReview, submitReview, submitting, invalidDraftIds, copyFeedbackPrompt, onJumpToTarget }: { pr: StoredPullRequest; files: PullFile[]; drafts: DraftComment[]; setDrafts: (drafts: DraftComment[]) => void; event: "COMMENT" | "APPROVE" | "REQUEST_CHANGES"; setEvent: (event: "COMMENT" | "APPROVE" | "REQUEST_CHANGES") => void; body: string; setBody: (body: string) => void; draftSaveStatus: "idle" | "saving" | "saved" | "error"; draftSaveError: string | null; retryDraftSave: () => void; editingDraftId: string | null; setEditingDraftId: (id: string | null) => void; archiveReview: (event: "COMMENT" | "APPROVE" | "REQUEST_CHANGES", body: string) => Promise<boolean>; discardReview: () => Promise<boolean>; submitReview: (event: "COMMENT" | "APPROVE" | "REQUEST_CHANGES", body: string) => Promise<boolean>; submitting: boolean; invalidDraftIds: Record<string, boolean>; copyFeedbackPrompt: (overallBody?: string) => Promise<void>; onJumpToTarget?: (target: Target) => void }) {
  const [finished, setFinished] = useState<"archived" | "published" | null>(null);
  const [copied, setCopied] = useState(false);
  ...
  const draftDescription = hasDrafts ? `${drafts.length} local draft ${drafts.length === 1 ? "comment" : "comments"} saved. Publish, archive, or keep editing.` : "Add an optional summary or click a diff line to draft a comment.";
  const [composing, setComposing] = useState(hasReviewContent);
  useEffect(() => {
    if (hasDrafts) setComposing(true);
  }, [hasDrafts]);
  function clearForm(nextFinished: "archived" | "published" | null): void {
    setDrafts([]);
    setBody("");
    setEvent("COMMENT");
  ...
  async function handlePublish(): Promise<void> {
    ...
    if (await submitReview(event, body)) clearForm("published");
  }
  async function handleArchive(): Promise<void> {
    if (archiving || !hasReviewContent) return;
    setArchiving(true);
    try {
      if (await archiveReview(event, body)) clearForm("archived");
    } finally {
      setArchiving(false);
    }
  }
  async function handleDiscard(): Promise<void> {
    const description = hasDrafts ? `${drafts.length} local draft ${drafts.length === 1 ? "comment" : "comments"}${body.trim().length > 0 ? " and the overall summary" : ""}` : "the local overall summary";
    if (discarding || !hasReviewContent || !window.confirm(`Discard ${description}?`)) return;
    setDiscarding(true);
    try {
      if (await discardReview()) clearForm(null);
    } finally {
      setDiscarding(false);
    }
  }
  async function copyDraftContext(): Promise<void> {
...
function AiReviewPanel({ prKey, prUrl, focusPanel, review, aiReviewHistory, aiReviewId, showAiReviewRecord, runReview, copyFeedbackPrompt, focusReview, focusScanHistory, focusScanId, showFocusScanRecord, runFocusReview, focusAreas, setActiveFocusAreaId, collapsedFocusAreaIds, setCollapsedFocusAreaIds, viewedFocusIds, setViewedFocusIds, saveFocusScan, openFiles, setOpenFiles }: PiPanelProps & { prKey: string; prUrl: string; focusPanel: () => void; focusAreas: FocusArea[]; setActiveFocusAreaId: (id: string | null) => void; collapsedFocusAreaIds: Record<string, boolean>; setCollapsedFocusAreaIds: DiffProps["setCollapsedFocusAreaIds"]; openFiles: Record<string, boolean>; setOpenFiles: (open: Record<string, boolean>) => void }) {
  ...
  const terminal = <section className="pi-terminal-session" aria-label="Pi terminal session">
    <div className="pi-terminal-session-head">
      ...
      }} aria-pressed={terminalFocused}>{terminalFocused ? "Show review context" : "Focus terminal"}</Button>
    </div>
    {terminalReview == null ? <Flash variant="danger">Open the pull request before starting its terminal.</Flash> : <React.Suspense fallback={<div className="pi-native-terminal-loading" role="status">Loading terminal…</div>}><PiTerminal prKey={prKey} headSha={terminalReview.headSha} onDraftReview={terminalReview.onDraftReview} /></React.Suspense>}
  </section>;
  ...
  return <section className={`panel ai-review${viewingHistory ? " viewing-history" : ""}${terminalFocused ? " terminal-focused" : ""}`}>
    ...
    <div className="pi-actions">
      ...
      <div className="pi-action">
        <Button onClick={() => void runReview()} disabled={review.running}>{review.running ? "Reviewing…" : reviewMessages.length > 0 ? "Refresh findings" : "Full review"}</Button>
        {review.running ? <AgentActivityLine activity={review.activity} /> : <span className="muted">Run a general code review. Use the terminal for follow-up investigation.</span>}
        {aiReviewHistory.length > 1 && <details className="pi-history-compact"><summary><span className="disclosure-chevron" aria-hidden="true">›</span>Review history ({aiReviewHistory.length})</summary><div className="pi-history-picker"><Select aria-label="Review history" value={selectedAiReviewId} onChange={(event) => showAiReviewRecord(aiReviewHistory.find((record) => record.id === event.target.value))}>{aiHistoryOptions}</Select>{viewingOlderAiReview && <Button variant="muted" className="pi-history-back" onClick={() => showAiReviewRecord(aiReviewHistory[0])}>Latest</Button>}</div></details>}
      </div>
    </div>
...
function DiagnosticsView({ diagnostics }: { diagnostics: Record<string, unknown> | null }) {
  ...
  return <div className="pi-card-stack">
    ...
    <PiCard title="Last agent prompt" count={lastPrompt?.chars ?? null}>
...
function PiRunDiagnostics({ aiReview, focusReview, focusAreaCount }: { aiReview: AiReview; focusReview: FocusReview; focusAreaCount: number }) {
  const reviewStatus = aiReview.running ? "running" : currentGeneralReviewText(aiReview).length > 0 ? "complete" : "idle";
  ...
  return <div className="pi-card-stack">
    <PiCard title="Pi runs">
      <div className="pi-mini-grid">
        <div><span>Full review</span><strong>{reviewStatus}</strong></div>
    ...
    {aiReview.text.length > 0 && <PiCard title="Latest Pi review answer" defaultOpen={false}><pre className="prompt-preview">{aiReview.text}</pre></PiCard>}
  </div>;
}
...
function reviewMemoryRecordTitle(record: ReviewMemoryRecord): string {
  return `${record.prKey} · ${record.disposition === "archived" ? "archived locally" : "published"} · ${record.event} · ${relativeTime(record.createdAt)}`;
}
...
function ReviewMemoryModal({ memory, loading, distilling, refresh, distill, close }: { memory: ReviewMemoryResponse | null; loading: boolean; distilling: boolean; refresh: () => void; distill: () => void; close: () => void }) {
  ...
  return <ModalShell open onOpenChange={(open) => { if (!open) close(); }} label="Review memory" className="pi-modal review-memory-modal">
    <header className="pi-modal-head">
      <div>
        <h2>Review memory</h2>
        <p className="muted">Published and locally archived reviews remain available as reusable feedback snapshots.</p>
      </div>
      <div className="pi-modal-head-actions">
