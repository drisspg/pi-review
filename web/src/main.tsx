...
function DraftView({ draft, drafts, setDrafts, editingDraftId, setEditingDraftId }: { draft: DraftComment; drafts: DraftComment[]; setDrafts: (drafts: DraftComment[]) => void; editingDraftId: string | null; setEditingDraftId: (id: string | null) => void }) {
  ...
  return <div className={`comment draft-card pending-draft${editing ? " editing" : ""}${removing ? " removing" : ""}`}><div className="thread-head"><div><strong>Pending draft</strong><span>{targetLabel(draft)}</span></div><div className="actions"><button className="draft-icon-button edit-draft-button" title={editing ? "Done editing" : "Edit draft"} onClick={() => setEditingDraftId(editing ? null : draft.id)}>{editing ? "Done" : "Edit"}</button><button className="draft-icon-button remove-draft-button" onClick={removeDraft} disabled={removing}>{removing ? "Removing…" : "Remove"}</button></div></div>{editing ? <textarea autoFocus value={draft.body} onChange={(event) => updateDraft(drafts, setDrafts, draft.id, event.target.value)} /> : <p>{draft.body}</p>}</div>;
}
