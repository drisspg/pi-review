...
function ConnectedRow({ file, row, comments, threads, setThreads, toggleThread, drafts, setDrafts, editingDraftId, setEditingDraftId, askThread, dragSelection, beginDrag, updateDrag, finishDrag, handleRowClick, prUrl, refreshGithubActivity, collapseSignal }: { file: PullFile; row: DiffRow; comments: PullReviewComment[]; threads: Record<string, Thread>; setThreads: DiffProps["setThreads"]; toggleThread: (target: Target, extend?: boolean) => void; drafts: DraftComment[]; setDrafts: (drafts: DraftComment[]) => void; editingDraftId: string | null; setEditingDraftId: (id: string | null) => void; askThread: (thread: Thread) => Promise<void>; dragSelection: DragSelection | null; beginDrag: (target: Target) => void; updateDrag: (target: Target) => void; finishDrag: (target: Target) => void; handleRowClick: (target: Target, extend: boolean) => void; prUrl: string; refreshGithubActivity: () => Promise<void>; collapseSignal: number }) {
...
function DiffRowView({ row, target, threads, setThreads, toggleThread, comments, drafts, setDrafts, editingDraftId, setEditingDraftId, askThread, dragSelection, beginDrag, updateDrag, finishDrag, handleRowClick, prUrl, refreshGithubActivity, collapseSignal }: { row: DiffRow; target: Target | null; threads: Record<string, Thread>; setThreads: DiffProps["setThreads"]; toggleThread: (target: Target, extend?: boolean) => void; comments: PullReviewComment[]; drafts: DraftComment[]; setDrafts: (drafts: DraftComment[]) => void; editingDraftId: string | null; setEditingDraftId: (id: string | null) => void; askThread: (thread: Thread) => Promise<void>; dragSelection: DragSelection | null; beginDrag: (target: Target) => void; updateDrag: (target: Target) => void; finishDrag: (target: Target) => void; handleRowClick: (target: Target, extend: boolean) => void; prUrl: string; refreshGithubActivity: () => Promise<void>; collapseSignal: number }) {
...
function AiReviewPanel({ review, setReview, runReview, sendMessage }: { review: AiReview; setReview: (review: AiReview) => void; runReview: () => Promise<void>; sendMessage: (message: string) => Promise<void> }) {
  ...
  return <>
    <section className="panel ai-review">
      <div className="thread-head"><h2>Pi review</h2><div className="actions"><Button variant="muted" onClick={() => setReview({ ...review, expanded: true, open: true })} disabled={!hasMessages && !review.open}>Focus</Button><Button variant="muted" onClick={() => setReview({ ...review, open: !review.open })}>{review.open ? "Collapse" : "Show"}</Button></div></div>
...
function FocusReviewPanel({ review, setReview, runReview, focusAreas, activeFocusAreaId, setActiveFocusAreaId, openFocusArea }: { review: FocusReview; setReview: (review: FocusReview) => void; runReview: () => Promise<void>; focusAreas: FocusArea[]; activeFocusAreaId: string | null; setActiveFocusAreaId: (id: string | null) => void; openFocusArea: (area: FocusArea) => void }) {
  const areaList = focusAreas.length > 0 ? <div className="focus-area-list">{focusAreas.map((area) => <button className={`focus-area-card${area.id === activeFocusAreaId ? " active" : ""}`} key={area.id} onClick={() => openFocusArea(area)}><strong>{area.path}:{area.startLine === area.endLine ? area.startLine : `${area.startLine}-${area.endLine}`}</strong><span>{area.title}</span></button>)}</div> : null;
  const body = review.text.length > 0 ? <>{areaList}<MarkdownText text={review.text} /></> : <p className="muted">Run an independent Pi session to find weird areas, codebase mismatches, and tradeoffs reviewers should investigate.</p>;
  return <>
